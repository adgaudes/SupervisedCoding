...
- **Delegation is priced before it runs.** A `small`, mechanical change to a single file of a few kilobytes, with no symbols to study, is refused once with the cheapest delegation cost measured in this repository, and the supervisor makes it itself; `delegateAnyway: true` overrides the refusal when the change needs a worker for reasons the extension cannot see. The floor is measured from recorded delegations, not assumed.
...
6. **Accept.** The supervisor reads the result (the diff is included) and calls `complete_task`. Acceptance needs evidence about the code as it stands: passing checks, or the independent review where one is required. If the supervisor changed the code after the checks passed, they run again; if it changed after a review, the whole task is reviewed again. A task no check applies to, and no review is required for, is accepted only with `manualReview: true` and the evidence in the summary. For large or critical tasks done in several steps (or whose last step was not reviewed), this first runs a review of the whole task diff.
...
- **La delega viene valutata prima di partire.** Una modifica meccanica al profilo `small`, su un solo file di pochi kilobyte e senza simboli da studiare, viene rifiutata una volta citando il costo della delega più economica misurata in questo repository, e il supervisore la fa da sé; `delegateAnyway: true` scavalca il rifiuto quando la modifica ha bisogno di un worker per ragioni che l'estensione non può vedere. Il pavimento è misurato dalle deleghe registrate, non assunto.
...
6. **Accettazione.** Il supervisore legge il risultato (il diff è incluso) e chiama `complete_task`. L'accettazione richiede evidenza sul codice così com'è: check che passano, o la review indipendente dove è richiesta. Se il supervisore ha modificato il codice dopo che i check sono passati, i check si rieseguono; se l'ha modificato dopo una review, l'intero task viene rivisto di nuovo. Un task a cui nessun check si applica e per cui nessuna review è richiesta si accetta solo con `manualReview: true` e l'evidenza nel riepilogo. Nei task large o critical svolti in più passi (o il cui ultimo passo non è stato rivisto) questo avvia prima una review del diff complessivo.
