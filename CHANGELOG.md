...
- An assessment that declares low risk on a consequential path is questioned once: a path whose segments or extension read as a migration or as security work (`sensitivePaths`) is refused with the reason, and the supervisor either states the risk and kind it stands behind or repeats the call unchanged, which then runs. The extension does not raise the profile by itself here, because only the supervisor knows whether this particular change touches the consequential part, and an automatic escalation would send every change near an `auth` directory to a flagship model. Matching is on whole path segments, never substrings, so `author.ts` is not authentication. The same path set is never questioned twice, so no call can loop.
- A delegation smaller than its own cost is refused once instead of run: a mechanical or documentation change at the `small` profile, on one authorized path that is a file of at most `tinyDelegationBytes` (8 KB by default) or does not exist yet, with no symbols named in the guide. The refusal quotes the cheapest delegation actually recorded in the repository, says to make the change with edit/write and verify it with `run_verification`, and starts nothing. `delegateAnyway: true` on the next call overrides it, so the supervisor keeps the last word when the change needs a worker for reasons the extension cannot see. `tinyDelegationBytes: 0` disables the check.
- The declared scope is corroborated with the authorized paths: two or more of them are multi-file work whatever the guide says, so an optimistic assessment cannot walk under the `medium` floor. Like the existing risk floors, it only ever raises the profile.
...
- `workerMaxTurns` keeps its defaults (40/80/120/160). Cutting them to 16/32/64/96 was tried and measured on a real refactor, and it made things worse: the worker stopped mid-extraction at 32 turns and the continuation, resuming a session whose prefix had only grown, brought the extraction to 5.3M tokens and $3.19 against 1.58M and $0.98 for the comparable extraction done before the cut. A cap bounds a runaway loop; it does not bound tokens, because it splits one long context into two long ones and adds a supervisor round trip. What bounds a worker's cost is how much of a file it has to hold, not how many turns it may take.
...
- A task could be accepted with no evidence about what was being accepted. A delegation with no allowlisted `VERIFY` command ends `unverified`, and at the `small` and `medium` profiles no review runs, so `complete_task` closed it on nothing; it now asks for a passing `run_verification`, or `manualReview: true` with the evidence in the summary, where no review is required. And evidence is tied to the code it was about: the working tree is recorded when a delegation's checks pass and when a review passes, and acceptance asks for the checks again, or reviews the whole task again, when the supervisor has changed the code since. Found by the self-audit; made more pressing by the supervisor now editing code itself.
- A check first listed in a later delegation of a task took its baseline from that delegation's start, after earlier delegations had already changed the code, so a regression introduced by step 1 was tolerated as pre-existing in step 2. Such a check now counts as green at the task start.
- The cold-start `low` effort for small mechanical work was invisible to learning: effort tuning counts only outcomes at the effort it tunes from, the configured one, so a reduction that kept failing would never have been withdrawn. It now yields as soon as tasks run at `low` in that repository and kind were poor, and whenever learning has already moved that combination's effort.
- Untracked files were read whole into synthetic diffs before any truncation, binaries included. Each now contributes at most 64 KB, and a binary file only its name and size.
- The worker prompt said "Run pertinent checks" while the worker notes said the extension runs them; it now says only not to claim a check it did not run.
- A misspelt entry in `independentReviewProfiles` ("larg") silently switched off the review it was meant to require. The configuration is now refused, naming the entry, like an invalid `verificationCommands` entry.
- The three gates added in this release were audited by the extension itself and three of them did not work as claimed. The refusal of a too-small delegation read the guide *after* validation, which adds a `SYMBOLS:` line of its own, so the "no symbols to study" condition never matched and the refusal was unreachable. The scope corroboration and the sensitive-path question both keyed on what the supervisor declared (`scope: "local"`, `risk: "low"`), so omitting the assessment — whose defaults are exactly local scope and medium risk — walked past both. All three now read the guide as written and treat an omitted assessment as the defaults it stands for, and the tests that missed this (they declared `SYMBOLS: none` and an explicit low risk) were replaced with ones that do not.
- Acceptance of a change the supervisor made itself no longer rests on stale or missing evidence: the check must have run on the working tree as it stands, it is recorded independently of the check-reuse cache (which holds nothing when reuse is off, and forgets a check that left files behind), and the last verdict per command counts, so a failure that was fixed and re-run does not block the rest of the prompt.
- Repository instruction files were lost when Pi was started from a symlinked or shortened spelling of the repository: Git answers with the real path while `cwd` kept the other form, and the walk from the Git root down to an authorized path compares them as text, so it stopped at once. A nested `AGENTS.md` beside the authorized path was never read and the root one was included twice, once labelled through a `../..` detour. Both are now resolved to their real path (symlinks, and 8.3 short names on Windows). Seen on the Windows CI runners; the same applies to `/tmp` on macOS.
